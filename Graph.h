...
private:
    ...
    void dfsHelper(const std::string& station, std::unordered_map<std::string, bool>& visited, std::vector<std::string>& traversal) const;
...
public:
    ...
    void addRoute(const std::string& from, const std::string& to, bool showMessage = true, bool allowCreateStations = false);

    // Adds a bidirectional weighted route for future scalability.
    void addWeightedRoute(const std::string& from, const std::string& to, int weight, bool showMessage = true, bool allowCreateStations = false);
    ...
    std::vector<std::string> findShortestRoute(const std::string& start, const std::string& destination) const;
